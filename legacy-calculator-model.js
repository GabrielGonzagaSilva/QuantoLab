...
function visibleControls(){return [...form.querySelectorAll('input,select')].filter(el=>!el.disabled&&el.type!=='hidden'&&!el.id.endsWith('Modo'));}
...
function buildModel(){
  ...
  redo.addEventListener('click',()=>{result.hidden=true;form.hidden=false;document.body.classList.remove('calculator-result-mode');if(matchMedia('(max-width:760px)').matches){form.scrollIntoView({behavior:'smooth',block:'start'});visibleControls()[0]?.focus({preventScroll:true});}});
  const sharedUrl=()=>{const values={};for(const control of visibleControls())values[control.id]=control.type==='checkbox'?control.checked:control.value;try{return `${location.origin}${location.pathname}#s=${btoa(encodeURIComponent(JSON.stringify(values)))}`;}catch{return `${location.origin}${location.pathname}`;}};
...
function restore(){if(!location.hash.startsWith('#s='))return false;try{const values=JSON.parse(decodeURIComponent(atob(location.hash.slice(3))));for(const control of form.querySelectorAll('input,select'))if(Object.prototype.hasOwnProperty.call(values,control.id)){if(control.type==='checkbox')control.checked=Boolean(values[control.id]);else control.value=String(values[control.id]);control.dispatchEvent(new Event('change',{bubbles:true}));}return true;}catch{return false;}}
function refreshModel(){
  ...
  for(const control of visibleControls()){const row=make('div','calculator-model__summary-row');row.append(make('span','',labelFor(control)),make('strong','',fieldValue(control)));summary.appendChild(row);}
