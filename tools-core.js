...
function setResultState(result,model,headline,summary,state,message=''){
  result.hidden=false;result.dataset.resultState=state;result.classList.toggle('model-active',state==='calculated');
  if(state==='waiting'){headline.textContent='Aguardando cálculo';summary.textContent='Preencha os dados e toque em Calcular para visualizar o resultado.';model.hidden=true;}
  if(state==='error'){headline.textContent='Revise os dados';summary.textContent=message;model.hidden=true;}
  if(state==='calculated')model.hidden=false;
}
...
function mount(){
  ...
  let last=null;
  const setWaiting=()=>{last=null;setResultState(result,model,headline,summary,'waiting');if(note){note.textContent='';note.hidden=true;}};
  ...
  model.querySelector('[data-redo-calculation]').addEventListener('click',()=>{setWaiting();showForm();});
  const calculate=()=>{
    const values=collect(form),validation=config.validate?.(values);if(validation){setResultState(result,model,headline,summary,'error',validation);return;}
    window.QuantoLabAnalytics?.track?.('calculation_started',{tool:slug});last=config.calc(values);setResultState(result,model,headline,summary,'calculated');headline.textContent=last.headline;summary.textContent=last.summary;rows?.replaceChildren();if(rows&&last.rows)for(const [rowLabel,value] of last.rows){const row=make('div','row');row.append(make('span','',rowLabel),make('strong','',value));rows.appendChild(row);}if(note){note.textContent=last.note||'';note.hidden=!last.note;}
  ...
  clearBtn.addEventListener('click',()=>{for(const input of form.querySelectorAll('[data-tool-input]')){if(input.type==='checkbox')input.checked=false;else if(input.tagName==='SELECT')input.selectedIndex=0;else input.value='';}setWaiting();showForm();});
