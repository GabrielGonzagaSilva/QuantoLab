...
function mount(){
  ...
  let last=null;
  function resetPreview(){result.hidden=false;result.classList.remove('model-active');model.hidden=true;if(details)details.hidden=true;if(shareHost)shareHost.hidden=true;if(headline)headline.textContent='Aguardando cálculo';if(summary)summary.textContent='Preencha os campos e toque em Calcular para ver a estimativa.';rows?.replaceChildren();if(note){note.textContent='';note.hidden=true;}renderSeries(model.querySelector('.calculator-model__visual'),[]);}
  ...
  model.querySelector('[data-redo-calculation]').addEventListener('click',()=>{model.hidden=true;showForm();});
  const calculate=()=>{
    const values=collect(form),validation=config.validate?.(values);if(validation){result.hidden=false;result.classList.remove('model-active');headline.textContent='Revise os dados';summary.textContent=validation;model.hidden=true;if(shareHost)shareHost.hidden=true;return;}
    window.QuantoLabAnalytics?.track?.('calculation_started',{tool:slug});last=config.calc(values);result.hidden=false;model.hidden=false;result.classList.add('model-active');if(shareHost)shareHost.hidden=false;headline.textContent=last.headline;summary.textContent=last.summary;rows?.replaceChildren();if(rows&&last.rows)for(const [rowLabel,value] of last.rows){const row=make('div','row');row.append(make('span','',rowLabel),make('strong','',value));rows.appendChild(row);}if(note){note.textContent=last.note||'';note.hidden=!last.note;}
    ...
    if(matchMedia('(max-width:760px)').matches){result.tabIndex=-1;result.focus({preventScroll:true});result.scrollIntoView({behavior:'smooth',block:'start'});}window.QuantoLabAnalytics?.track?.('calculation_completed',{tool:slug});
  };
  ...
  clearBtn.addEventListener('click',()=>{for(const input of form.querySelectorAll('[data-tool-input]')){if(input.type==='checkbox')input.checked=false;else if(input.tagName==='SELECT')input.selectedIndex=0;else input.value='';}last=null;resetPreview();showForm();});
