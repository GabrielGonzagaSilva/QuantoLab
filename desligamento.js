...
$('copiar').addEventListener('click',async()=>{const encoded=encodeState(),url=encoded?`${location.origin}${location.pathname}#s=${encoded}`:`${location.origin}${location.pathname}`,text=last?`Rescisão estimada: ${money(last.directNet)}. ${url}`:url;try{await navigator.clipboard.writeText(text);$('shareStatus').textContent='Link copiado.';if(last)globalThis.QuantoLabAnalytics?.track?.('result_shared',{tool:TOOL,method:'copy'});setTimeout(()=>$('shareStatus').textContent='',1800);}catch{$('shareStatus').textContent='Não foi possível copiar o link.';}});
$('compartilhar').addEventListener('click',async()=>{const encoded=encodeState(),url=encoded?`${location.origin}${location.pathname}#s=${encoded}`:`${location.origin}${location.pathname}`,text=last?`Rescisão estimada: ${money(last.directNet)}`:'Calculadora de rescisão';if(navigator.share){try{await navigator.share({title:'Calculadora de rescisão',text,url});if(last)globalThis.QuantoLabAnalytics?.track?.('result_shared',{tool:TOOL,method:'native'});}catch{}}else $('copiar').click();});
