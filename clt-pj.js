...
function calculate(event){
  ...
  globalThis.QuantoLabAnalytics?.track?.('calculation_started',{tool:TOOL});
  ...
  globalThis.QuantoLabAnalytics?.track?.('calculation_completed',{tool:TOOL});
}
...
$('copiar').addEventListener('click',async()=>{const url=shareUrl(),text=last?`CLT x PJ: valor PJ equivalente ${money(last.pjGross)}. ${url}`:url;try{await navigator.clipboard.writeText(text);$('shareStatus').textContent='Link copiado.';if(last)globalThis.QuantoLabAnalytics?.track?.('result_shared',{tool:TOOL,method:'copy'});setTimeout(()=>$('shareStatus').textContent='',1800);}catch{$('shareStatus').textContent='Não foi possível copiar o link.';}});
$('compartilhar').addEventListener('click',async()=>{const url=shareUrl(),text=last?`Valor PJ equivalente: ${money(last.pjGross)}`:'Calculadora CLT x PJ';if(navigator.share){try{await navigator.share({title:'Calculadora CLT x PJ',text,url});if(last)globalThis.QuantoLabAnalytics?.track?.('result_shared',{tool:TOOL,method:'native'});}catch{}}else $('copiar').click();});
