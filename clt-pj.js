...
function setResultState(state,message=''){
  ...
  if(state==='waiting'){$('pjEquivalente').textContent='Aguardando cálculo';$('status').textContent='Preencha os dados e toque em Calcular para visualizar o resultado.';}
  if(state==='error'){$('pjEquivalente').textContent='Revise os dados';$('status').textContent=message;}
}
function showForm(){form.hidden=false;document.body.classList.remove('calculator-result-mode');if(matchMedia('(max-width:760px)').matches){form.scrollIntoView({behavior:'smooth',block:'start'});$('cltSalario').focus({preventScroll:true});}}
...
function calculate(event){
  ...
  if(!salary){setResultState('error','Informe o salário bruto CLT para calcular.');return;}
  ...
  enterResultMode();window.QuantoLabAnalytics?.track?.('calculation_completed',{tool:'comparador-profissional'});
}
function clearAll(){form.reset();$('cltSalario').value='';$('valeTransporte').value='0';$('valeRefeicao').value='0';$('outrosBeneficios').value='0';$('simples').value='6';last=null;setResultState('waiting');showForm();}
